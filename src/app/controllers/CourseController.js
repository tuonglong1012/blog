...
class CourseController {
  // [GET] /courses/course -> API danh sách khóa học có phân trang
  course(req, res, next) {
    let { page, limit } = req.query;
    page = parseInt(page) || 1; // Mặc định trang 1
    limit = parseInt(limit) || 5; // Mặc định 5 khóa học mỗi trang

    Course.countDocuments() // Đếm tổng số khóa học
      .then((totalCourses) => {
  ...
  // [POST] /courses/create -> Lưu khóa học vào DB
  store(req, res, next) {
